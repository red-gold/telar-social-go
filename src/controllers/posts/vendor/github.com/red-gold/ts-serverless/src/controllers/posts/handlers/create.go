...
import (
	...
	"github.com/red-gold/ts-serverless/src/constants"
	domain "github.com/red-gold/ts-serverless/src/domain/posts"
	models "github.com/red-gold/ts-serverless/src/models/posts"
	service "github.com/red-gold/ts-serverless/src/services/posts"
)
...
func CreatePostHandle(db interface{}) func(server.Request) (handler.Response, error) {
	...
	return func(req server.Request) (handler.Response, error) {
		...
		if err := postService.SavePost(newPost); err != nil {
			errorMessage := fmt.Sprintf("Save Post Error %s", err.Error())
			return handler.Response{StatusCode: http.StatusInternalServerError, Body: utils.MarshalError("savePostError", errorMessage)}, nil
		}
